      ...
      <div className="bg-white rounded-2xl card-responsive shadow-lg border border-slate-200">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
          ...
          <Button 
            ...
            className="bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700 text-white button-responsive rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 w-full sm:w-auto"
          >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button
              ...
              className={`button-responsive ${filter === 'all' ? 'bg-gradient-to-r from-green-600 to-blue-600 text-white' : 'hover:bg-slate-100'}`}
            >
              All ({exams.length})
            </Button>
            <Button
      ...
      {filteredExams.length === 0 ? (
        <Card className="border-0 shadow-lg">
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            {!searchTerm && (
              <Link href="/proctor/exams/new">
                <Button className="bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700 text-white px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200">
            ...
            <Card key={exam.id} className="group hover:shadow-xl transition-all duration-300 border-0 shadow-lg bg-white">
              <CardHeader className="pb-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <CardTitle className="line-clamp-2 text-slate-900 group-hover:text-green-600 transition-colors">{exam.title}</CardTitle>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    <Link href={`/proctor/exams/${exam.id}`} className="flex-1">
                      <Button variant="outline" size="sm" className="w-full hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200">
                        <Eye className="mr-2 h-4 w-4" />
                        View
                      </Button>
                    </Link>
                    <Link href={`/proctor/exams/${exam.id}/edit`} className="flex-1">
                      <Button variant="outline" size="sm" className="w-full hover:bg-green-50 hover:text-green-600 hover:border-green-200">
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid gap-3 py-4">
            {[
              ...
              <Link 
                key={subject.name}
                href={`/proctor/exams/new?type=mock&course=criminology&subject=${encodeURIComponent(subject.name.toLowerCase().replace(/\s+/g, '-'))}`}
              >
                <Button 
